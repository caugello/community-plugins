...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [multiSourceSecurityViewerPlugin],
    });

    await request(server)
      .get('/api/multi-source-security-viewer/todos')
      .expect(200, {
        items: [],
      });

    const createRes = await request(server)
      .post('/api/multi-source-security-viewer/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get(`/api/multi-source-security-viewer/todos/${createdTodoItem.id}`)
